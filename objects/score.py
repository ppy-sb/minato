...
class Score(BaseModel):
    ...
    async def from_sql(score_id: int, mode: GameMode):
        ...
        return Score(
            accuracy=float(row["acc"]) / 100,
            best_id=row["id"],
            id=row["id"],
            created_at=row["play_time"],
            max_combo=row["max_combo"],
            mode=mode.name,
            mode_int=mode.as_vanilla,
            mods=Mods(row["mods"]).as_list(),
            passed=(row["grade"] != "F"),
            perfect=bool(row["perfect"]),
            pp=row["pp"],
            rank=row["grade"].replace("X", "SS"),
            score=row["score"],
...
async def get_best_scores(user_id: int, include_fails: bool, mode: str, limit: int, offset: int):
    ...
    async with db_context() as (_, cur):
        ...
        for i, row in enumerate(await cur.fetchall()):
            ...
            score.weight = {"percentage": percentage, "pp": pp}
            result.append(score)
...
async def get_recent_scores(user_id: int, include_fails: bool, mode: str, limit: int, offset: int):
    ...
    async with db_context() as (_, cur):
        await cur.execute(
            f"select id from scores where userid = %s and mode = %s "
